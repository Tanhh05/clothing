  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/products?page=0&size=20&sortBy=createdAt&direction=desc&q=${searchValue}`
      );
      const fetchedProducts: itemType[] = (res.data.content || []).map(
        mapApiProductToItem
      );
