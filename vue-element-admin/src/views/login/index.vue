<template>
  <div class="login-container">
    ...
    <el-dialog title="Hoặc đăng nhập bằng" :visible.sync="showDialog">
      Không thể mô phỏng đầy đủ ở môi trường local, vui lòng tích hợp theo nghiệp vụ thực tế của bạn.
      <br>
      <br>
      <br>
      <social-sign />
  ...
  data() {
    const validateUsername = (rule, value, callback) => {
      if (!validUsername(value)) {
        callback(new Error('Tên đăng nhập hoặc email phải có ít nhất 3 ký tự'))
      } else {
        callback()
      }
    }
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          ...
          console.log('Lỗi khi gửi biểu mẫu')
          return false
        }
      })
    },
