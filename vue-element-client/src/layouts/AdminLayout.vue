...
const runGlobalSearch = async (query) => {
  ...
  try {
    const [productState, categoryState, orderState] = await Promise.allSettled([
      searchApi.searchProducts(q, 6),
      categoryApi.getCategories({ page: 0, size: 100, sortBy: "id", direction: "desc" }),
      orderApi.getAdminOrders({ page: 0, size: 6, sortBy: "id", direction: "desc", q })
    ]);

    const productsSource = productState.status === "fulfilled" ? productState.value?.data : [];
    const products = Array.isArray(productsSource)
      ? productsSource.slice(0, 6)
      : Array.isArray(productsSource?.content)
        ? productsSource.content.slice(0, 6)
        : [];

    const categorySource = categoryState.status === "fulfilled" ? categoryState.value?.data : null;
    const allCategories = Array.isArray(categorySource?.content)
      ? categorySource.content
    ...
    const orderSource = orderState.status === "fulfilled" ? orderState.value?.data : null;
    const orders = Array.isArray(orderSource?.content)
      ? orderSource.content.slice(0, 6)
      : Array.isArray(orderSource)
        ? orderSource.slice(0, 6)
        : [];
...
const handleSearchSubmit = () => {
  searchOpen.value = true;
  if (searchTimer) clearTimeout(searchTimer);
  runGlobalSearch(searchQuery.value);
};
